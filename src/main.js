...
function renderGuide() {
  return '<main class="support-page guide-page"><div class="support-content"><button class="support-back" data-page="review">← Back to sheet</button><div class="support-heading"><p class="eyebrow">PRINT GUIDE</p><h1>Print once on plain paper.</h1><p>A quick test confirms scale and alignment before you use sticker stock.</p></div><div class="guide-list"><article><span>01</span><div><h2>Print at Actual Size</h2><p>Choose A4 and 100% / Actual Size. Do not use Fit, Shrink, or Scale to Printable Area.</p></div></article><article><span>02</span><div><h2>Lay it over the stock</h2><p>Hold the plain-paper test behind the sticker sheet against a bright window. Text should sit inside each label.</p></div></article><article><span>03</span><div><h2>Then use sticker stock</h2><p>Feed one sheet at a time and use the manual or multipurpose tray when available.</p></div></article><article class="warning-card"><span>!</span><div><h2>If it is shifted</h2><p>Check that scaling is still 100%. A small consistent offset belongs in the printer driver or manufacturer settings.</p></div></article></div></div>' + renderSupportFooter() + '</main>';
}

function renderPrivacy() {
  return '<main class="support-page privacy-page"><div class="support-content"><button class="support-back" data-page="review">← Back to sheet</button><div class="support-heading"><p class="eyebrow">PRIVACY</p><h1>Your data stays in this tab.</h1><p>Pasted rows are processed in this browser. Nothing is uploaded or saved to a database.</p></div><div class="privacy-list"><article><span>01</span><div><h2>Local processing</h2><p>Your pasted rows stay in this browser tab. The PDF is assembled on this device and downloaded directly to you.</p></div></article><article><span>02</span><div><h2>No account or upload</h2><p>There is no sign-in, participant database, file upload, or shared workspace.</p></div></article><article><span>03</span><div><h2>Memory-only working data</h2><p>Refresh or close this tab and the working data is gone.</p></div></article></div></div>' + renderSupportFooter() + '</main>';
}
